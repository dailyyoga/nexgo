// Package dlqmetrics is the Prometheus adapter for the nexgo/dlq package's
// infra counters. It lives in its own subpackage (not in metrics) on purpose:
// importing it transitively pulls in nexgo/dlq -> nexgo/kafka and the cgo
...
// Register registers scrape-time collectors that expose a dlq recorder's infra
// counters on reg. The values are read live at scrape time via
...
// is ignored so calling Register twice on the same registry is safe.
func Register(reg *metrics.Registry, s dlq.Stats) {
	if s == nil {
		return
	}

	metrics.RegisterOrExisting(reg, prometheus.NewCounterFunc(prometheus.CounterOpts{
